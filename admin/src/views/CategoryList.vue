<template>
  <div class="categories-page">
    <div class="page-header">
      <div>
        <h1>Categorias</h1>
        <p class="subtitle">Gerencie as categorias das notícias</p>
      </div>
      <router-link v-if="canAccess('categories')" to="/categories/new" class="btn-primary">
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="category in categories" :key="category.id">
            ...
            <td>
              <span class="badge" :class="category.active ? 'badge-success' : 'badge-muted'">
                {{ category.active ? 'Ativa' : 'Inativa' }}
              </span>
            </td>
            <td>{{ category.order }}</td>
            <td class="actions">
              <router-link v-if="canAccess('categories')" :to="`/categories/${category.id}/edit`" class="btn-icon" title="Editar">
                <Pencil :size="16" />
              </router-link>
              <button v-if="canAccess('categories')" class="btn-icon btn-danger" title="Excluir" @click="confirmDelete(category)">
                <Trash2 :size="16" />
              </button>
            </td>
          </tr>
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await api.delete(`/api/categories/${deleteTarget.value.id}`)
